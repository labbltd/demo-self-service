        ...
        <cds-radio-group [direction]="container.config.datasource.length > 2 ? 'column' : 'row'">
            @for (option of container.config.datasource; track option.key) {
              <cds-radio [label]="option.value" [for]="container.id+option.key">
                  <input cdsInput type="radio" [name]="container.id" [id]="container.id+option.key" 
                      [value]="option.key" [checked]="option.key === container.config.value" (change)="change(option.key)">
              </cds-radio>
            }
        </cds-radio-group>
    </cds-form-field>
